...
class MonHocController:
    ...
    @classmethod
    def getInstance(cls):
        if (MonHocController._instance):
            return MonHocController._instance
        MonHocController._instance = MonHocController()
        return MonHocController._instance

    def __init__(self, model: MonHocModel, view: MonHocView):
        self._model = model
        self._view = view
        self._tree = self._view.tree
        self._view.ma_mh.bind('<KeyRelease>', self.ma_key_text_change)
        self._view.ten_mh.bind('<KeyRelease>', self.field_text_change)
        self._view.tclt.bind('<KeyRelease>', self.field_text_change)
        self._view.tcth.bind('<KeyRelease>', self.field_text_change)
    ...
    def add_item(self):
        ...
        ma__mh = item["MAMH"]
        is_exist = self._model.is_ma_exist(ma__mh)
        if is_exist == True:
    ...
    def get_selected_item(self, event=None):
        ...
        if hv != None:
            ...
            self._view.set_ten_mh(ten_mh)
            self._view.set_tclt(tclt)
            self._view.set_tcth(tcth)
